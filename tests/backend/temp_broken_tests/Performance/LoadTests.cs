...
public class LoadTests : DatabaseTestBase
{
    ...
    public async Task ConcurrentAnalysis_ShouldHandleLoad()
    {
        ...
        foreach (var fileId in fileIds)
        {
            var analysisResult = new AnalysisResult
            {
                ...
                Result = $"{{\"subject\":\"Mathematics\",\"topics\":[\"Algebra{i}\"]}}",
    ...
    public async Task Caching_ShouldHandleHighLoad()
    {
        ...
        {
            var prompt = $"Load test prompt {i % 10}"; // Only 10 unique prompts
            var userId = i % 5; // Only 5 unique users
            tasks.Add(cachedService.GenerateStudyGuideAsync(prompt, userId));
        }
        ...
        var expectedCalls = 10 * 5; // 10 unique prompts * 5 unique users
