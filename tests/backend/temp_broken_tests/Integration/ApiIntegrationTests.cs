...
public class ApiIntegrationTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    ...
    public async Task CompleteUserFlow_ShouldWorkEndToEnd()
    {
        ...
        var updateRequest = new UpdateProfileRequest
        {
            ...
            PreferredSubjects = new List<string> { "Mathematics", "Science" }
        };

        var updateJson = JsonSerializer.Serialize(updateRequest);
        var updateContent = new StringContent(updateJson, Encoding.UTF8, "application/json");

        var updateResponse = await _client.PutAsync($"/api/user/profile/{userId}", updateContent);
        updateResponse.EnsureSuccessStatusCode();

        // Step 5: Create a study session
        var sessionRequest = new CreateStudySessionRequest
        {
            Subject = "Mathematics",
            TopicsStudied = new List<string> { "Algebra", "Calculus" },
            Duration = 60
        };
        ...
        var sessionResponse = await _client.PostAsync($"/api/user/study-session/{userId}", sessionContent);
        sessionResponse.EnsureSuccessStatusCode();

        // Step 6: Get study sessions
        var sessionsResponse = await _client.GetAsync($"/api/user/study-sessions/{userId}");
        sessionsResponse.EnsureSuccessStatusCode();

        var sessionsResult = await sessionsResponse.Content.ReadAsStringAsync();
        var sessionsData = JsonSerializer.Deserialize<JsonElement[]>(sessionsResult);

        Assert.True(sessionsData.Length > 0);
        Assert.Equal("Mathematics", sessionsData[0].GetProperty("subject").GetString());
    ...
    public async Task CompleteStudyGuideFlow_ShouldWorkEndToEnd()
    {
        ...
        var studyGuideRequest = new GenerateStudyGuideRequest
        {
            AnalysisId = analysisId,
            FocusAreas = new List<string> { "Algebra", "Calculus" },
