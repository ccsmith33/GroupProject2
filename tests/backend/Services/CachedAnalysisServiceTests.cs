using Microsoft.Extensions.Caching.Memory;
using Microsoft.Extensions.Logging;
using Moq;
using StudentStudyAI.Models;
using StudentStudyAI.Services;
using Xunit;

namespace StudentStudyAI.Tests.Services;

public class CachedAnalysisServiceTests : IDisposable
{
    private readonly Mock<AnalysisService> _mockAnalysisService;
    private readonly IMemoryCache _memoryCache;
    private readonly Mock<ILogger<CachedAnalysisService>> _mockLogger;
    private readonly CachedAnalysisService _cachedAnalysisService;

    public CachedAnalysisServiceTests()
    {
        _mockAnalysisService = new Mock<AnalysisService>(
            Mock.Of<OpenAIService>(),
            Mock.Of<DatabaseService>(),
            Mock.Of<ContextService>(),
            Mock.Of<ILogger<AnalysisService>>());
        
        _memoryCache = new MemoryCache(new MemoryCacheOptions());
        _mockLogger = new Mock<ILogger<CachedAnalysisService>>();
        
        _cachedAnalysisService = new CachedAnalysisService(
            _mockAnalysisService.Object,
            _memoryCache,
            _mockLogger.Object);
    }

    [Fact]
    public void CachedAnalysisService_ShouldBeCreated()
    {
        // Arrange & Act
        var service = new CachedAnalysisService(
            _mockAnalysisService.Object,
            _memoryCache,
            _mockLogger.Object);

        // Assert
        Assert.NotNull(service);
    }

    [Fact]
    public async Task GetCachedAnalysis_ShouldReturnCachedResult()
    {
        // Arrange
        var fileAnalysis = new FileAnalysis
        {
            Subject = "Mathematics",
            Topic = "Algebra",
            Difficulty = "Intermediate"
        };
        var userId = 1;
        var expectedStudyGuide = new StudyGuide
        {
            Title = "Cached Study Guide",
            Content = "Test content",
            KeyPoints = new List<string> { "Point 1", "Point 2" },
            Summary = "Test summary"
        };

        // First call - should cache the result
        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId))
            .ReturnsAsync(expectedStudyGuide);

        // Act - First call
        var result1 = await _cachedAnalysisService.GenerateStudyGuideAsync("test prompt", userId);

        // Act - Second call (should use cache)
        var result2 = await _cachedAnalysisService.GenerateStudyGuideAsync("test prompt", userId);

        // Assert
        Assert.Equal(expectedStudyGuide.Title, result1.Title);
        Assert.Equal(expectedStudyGuide.Title, result2.Title);
        
        // Should only call the underlying service once
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId), Times.Once);
    }

    [Fact]
    public async Task GetCachedAnalysis_ShouldCacheNewResult()
    {
        // Arrange
        var fileAnalysis = new FileAnalysis
        {
            Subject = "Science",
            Topic = "Physics",
            Difficulty = "Advanced"
        };
        var userId = 1;
        var expectedStudyGuide = new StudyGuide
        {
            Title = "New Study Guide",
            Content = "New content",
            KeyPoints = new List<string>(),
            Summary = "New summary"
        };

        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId))
            .ReturnsAsync(expectedStudyGuide);

        // Act
        var result = await _cachedAnalysisService.GenerateStudyGuideAsync("new prompt", userId);

        // Assert
        Assert.Equal(expectedStudyGuide.Title, result.Title);
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId), Times.Once);
    }

    [Fact]
    public async Task GetCachedAnalysis_ShouldExpireOldResults()
    {
        // Arrange
        var fileAnalysis = new FileAnalysis
        {
            Subject = "Mathematics",
            Topic = "Calculus",
            Difficulty = "Advanced"
        };
        var userId = 1;
        var expectedStudyGuide = new StudyGuide
        {
            Title = "Expiring Study Guide",
            Content = "Expiring content",
            KeyPoints = new List<string>(),
            Summary = "Expiring summary"
        };

        // Create a service with short cache expiration
        var shortCache = new MemoryCache(new MemoryCacheOptions
        {
            Clock = new TestClock(),
            ExpirationScanFrequency = TimeSpan.FromMilliseconds(100)
        });

        var service = new CachedAnalysisService(
            _mockAnalysisService.Object,
            shortCache,
            _mockLogger.Object);

        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId))
            .ReturnsAsync(expectedStudyGuide);

        // Act - First call
        var result1 = await service.GenerateStudyGuideAsync("expiring prompt", userId);

        // Wait for cache to expire
        await Task.Delay(200);

        // Act - Second call (should not use cache)
        var result2 = await service.GenerateStudyGuideAsync("expiring prompt", userId);

        // Assert
        Assert.Equal(expectedStudyGuide.Title, result1.Title);
        Assert.Equal(expectedStudyGuide.Title, result2.Title);
        
        // Should call the underlying service twice
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId), Times.Exactly(2));
    }

    [Fact]
    public async Task InvalidateCache_ShouldClearCache()
    {
        // Arrange
        var fileAnalysis = new FileAnalysis
        {
            Subject = "Mathematics",
            Topic = "Statistics",
            Difficulty = "Intermediate"
        };
        var userId = 1;
        var expectedStudyGuide = new StudyGuide
        {
            Title = "Cache Test Study Guide",
            Content = "Cache test content",
            KeyPoints = new List<string>(),
            Summary = "Cache test summary"
        };

        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId))
            .ReturnsAsync(expectedStudyGuide);

        // Act - First call (should cache)
        var result1 = await _cachedAnalysisService.GenerateStudyGuideAsync("cache test prompt", userId);

        // Act - Invalidate cache (simulate by clearing memory cache)
        _memoryCache.Remove($"study_guide_{userId}_cache test prompt".GetHashCode());

        // Act - Second call (should not use cache)
        var result2 = await _cachedAnalysisService.GenerateStudyGuideAsync("cache test prompt", userId);

        // Assert
        Assert.Equal(expectedStudyGuide.Title, result1.Title);
        Assert.Equal(expectedStudyGuide.Title, result2.Title);
        
        // Should call the underlying service twice
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), userId), Times.Exactly(2));
    }

    [Fact]
    public async Task GenerateQuizAsync_ShouldUseCache()
    {
        // Arrange
        var fileAnalysis = new FileAnalysis
        {
            Subject = "Mathematics",
            Topic = "Algebra",
            Difficulty = "Intermediate"
        };
        var userId = 1;
        var expectedQuiz = new Quiz
        {
            Title = "Cached Quiz",
            QuestionsList = new List<QuizQuestion>
            {
                new QuizQuestion
                {
                    Question = "Test question?",
                    Options = new List<string> { "A", "B", "C", "D" },
                    CorrectAnswerIndex = 0,
                    Explanation = "Test explanation"
                }
            }
        };

        _mockAnalysisService.Setup(x => x.GenerateQuizAsync(It.IsAny<string>(), userId))
            .ReturnsAsync(expectedQuiz);

        // Act - First call
        var result1 = await _cachedAnalysisService.GenerateQuizAsync("quiz test prompt", userId);

        // Act - Second call (should use cache)
        var result2 = await _cachedAnalysisService.GenerateQuizAsync("quiz test prompt", userId);

        // Assert
        Assert.Equal(expectedQuiz.Title, result1.Title);
        Assert.Equal(expectedQuiz.Title, result2.Title);
        
        // Should only call the underlying service once
        _mockAnalysisService.Verify(x => x.GenerateQuizAsync(It.IsAny<string>(), userId), Times.Once);
    }

    [Fact]
    public async Task GenerateConversationalResponseAsync_ShouldUseCache()
    {
        // Arrange
        var conversation = new Conversation
        {
            Id = 1,
            UserId = 1,
            Prompt = "Test prompt",
            Response = "",
            Subject = "Mathematics",
            Topic = "Algebra"
        };
        var prompt = "Test prompt";
        var expectedResponse = "Cached conversational response";

        _mockAnalysisService.Setup(x => x.GenerateConversationalResponseAsync(It.IsAny<string>(), It.IsAny<int>()))
            .ReturnsAsync(expectedResponse);

        // Act - First call
        var result1 = await _cachedAnalysisService.GenerateConversationalResponseAsync(prompt, 1);

        // Act - Second call (should use cache)
        var result2 = await _cachedAnalysisService.GenerateConversationalResponseAsync(prompt, 1);

        // Assert
        Assert.Equal(expectedResponse, result1);
        Assert.Equal(expectedResponse, result2);
        
        // Should only call the underlying service once
        _mockAnalysisService.Verify(x => x.GenerateConversationalResponseAsync(It.IsAny<string>(), It.IsAny<int>()), Times.Once);
    }

    [Fact]
    public async Task DifferentUsers_ShouldHaveSeparateCaches()
    {
        // Arrange
        var fileAnalysis = new FileAnalysis
        {
            Subject = "Mathematics",
            Topic = "Algebra",
            Difficulty = "Intermediate"
        };
        var studyGuide1 = new StudyGuide
        {
            Title = "User 1 Study Guide",
            Content = "User 1 content",
            KeyPoints = new List<string>(),
            Summary = "User 1 summary"
        };

        var studyGuide2 = new StudyGuide
        {
            Title = "User 2 Study Guide",
            Content = "User 2 content",
            KeyPoints = new List<string>(),
            Summary = "User 2 summary"
        };

        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), 1))
            .ReturnsAsync(studyGuide1);
        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), 2))
            .ReturnsAsync(studyGuide2);

        // Act
        var result1 = await _cachedAnalysisService.GenerateStudyGuideAsync("shared prompt", 1);
        var result2 = await _cachedAnalysisService.GenerateStudyGuideAsync("shared prompt", 2);

        // Assert
        Assert.Equal("User 1 Study Guide", result1.Title);
        Assert.Equal("User 2 Study Guide", result2.Title);
        
        // Should call the underlying service twice (once for each user)
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), 1), Times.Once);
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync(It.IsAny<string>(), 2), Times.Once);
    }

    [Fact]
    public async Task DifferentFileAnalyses_ShouldHaveSeparateCaches()
    {
        // Arrange
        var userId = 1;
        var fileAnalysis1 = new FileAnalysis
        {
            Subject = "Mathematics",
            Topic = "Algebra",
            Difficulty = "Intermediate"
        };
        var fileAnalysis2 = new FileAnalysis
        {
            Subject = "Science",
            Topic = "Physics",
            Difficulty = "Advanced"
        };
        var studyGuide1 = new StudyGuide
        {
            Title = "Study Guide 1",
            Content = "Content 1",
            KeyPoints = new List<string>(),
            Summary = "Summary 1"
        };

        var studyGuide2 = new StudyGuide
        {
            Title = "Study Guide 2",
            Content = "Content 2",
            KeyPoints = new List<string>(),
            Summary = "Summary 2"
        };

        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync("prompt 1", userId))
            .ReturnsAsync(studyGuide1);
        _mockAnalysisService.Setup(x => x.GenerateStudyGuideAsync("prompt 2", userId))
            .ReturnsAsync(studyGuide2);

        // Act
        var result1 = await _cachedAnalysisService.GenerateStudyGuideAsync("prompt 1", userId);
        var result2 = await _cachedAnalysisService.GenerateStudyGuideAsync("prompt 2", userId);

        // Assert
        Assert.Equal("Study Guide 1", result1.Title);
        Assert.Equal("Study Guide 2", result2.Title);
        
        // Should call the underlying service twice (once for each prompt)
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync("prompt 1", userId), Times.Once);
        _mockAnalysisService.Verify(x => x.GenerateStudyGuideAsync("prompt 2", userId), Times.Once);
    }

    public void Dispose()
    {
        _memoryCache?.Dispose();
    }

    // Test clock for controlling cache expiration
    private class TestClock : Microsoft.Extensions.Internal.ISystemClock
    {
        private DateTimeOffset _utcNow = DateTimeOffset.UtcNow;

        public DateTimeOffset UtcNow
        {
            get => _utcNow;
            set => _utcNow = value;
        }
    }
}
