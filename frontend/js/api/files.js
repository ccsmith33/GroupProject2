...
class FilesAPI {
  ...
  async getGroupedFiles(userId) {
    return apiCallWithRetry(async () => {
      const response = await fetch(
        `${this.baseURL}${CONFIG.ENDPOINTS.FILES.GROUPED}/${userId}/grouped`,
        {
          method: "GET",
          headers: {
            ...authState.getAuthHeader(),
  ...
  async downloadFile(fileId) {
    return apiCallWithRetry(async () => {
      const response = await fetch(
        `${this.baseURL}${CONFIG.ENDPOINTS.FILES.DOWNLOAD}/${fileId}/download`,
        {
          method: "GET",
          headers: {
            ...authState.getAuthHeader(),
      ...
      if (!response.ok) {
        ...
        const error = new Error(
          errorData.message || errorData.detail || "Download failed"
        );
        error.status = response.status;
        throw error;
      }
  ...
  // Reorganize files (re-detect and regroup)
  async reorganizeFiles(userId) {
    return apiCallWithRetry(async () => {
      const response = await fetch(
        `${this.baseURL}${CONFIG.ENDPOINTS.FILES.REORGANIZE}/${userId}`,
        {
          method: "POST",
          headers: {
            ...authState.getAuthHeader(),
      ...
      if (!response.ok) {
        const error = new Error(
          data.message || data.detail || "Failed to reorganize files"
        );
        error.status = response.status;
        throw error;
      }
